...
export class CorteCajaController {
    ...
    static getAll = async (req: Request, res: Response) => {
        try {
            ...
            console.error(error);
            res.status(500).json({ mensaje: "Error al encontrar todos los cortes de caja." });
        }
    }
    ...
    static getCantidadCortesPorCaja = async (req: Request, res: Response) => {
        try {
            ...
            const cantidadCortes = await CorteCajaService.getCantidadCortesPorCaja(id_caja);
            res.status(200).json({ cantidad: cantidadCortes });
        } catch (error) {
            console.error(error);
            res.status(500).json({ mensaje: "Error al obtener la cantidad de cortes de caja." });
        }
    }
    ...
    static create = async (req: Request, res: Response) => {
        try {
            const data = req.body;
            const nuevoCorte = await CorteCajaService.createCorteCaja(data);
    ...
    static update = async (req: Request, res: Response) => {
        try {
            const { id_corte } = req.params;
            const data = req.body;
            const corteActualizado = await CorteCajaService.updateCorteCaja(id_corte, data);
