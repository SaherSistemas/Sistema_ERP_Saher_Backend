...
export class NotasCreditoController {
    ...
    static darEntradaInventario = async (req: AuthedRequest, res: Response) => {
        try {
            ...
            const { id_compra_proveedor, productos } = req.body;
            if (!id_compra_proveedor || !Array.isArray(productos) || productos.length === 0) {
                res.status(400).json({ message: "id_compra_proveedor y productos son requeridos" });
            }
            const resultado = await NotasCreditoProveedorService.darEntradaInventario({ id_compra_proveedor, id_empresa: req.user?.id_empresa, productos });
