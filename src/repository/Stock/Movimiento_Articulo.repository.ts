...
import Movimientos_Articulos from '../../models/Stock/Movimientos_Articulos.model';
...
export const Movimiento_ArticuloRepository = {
    getAll: async () => {
        return await Movimientos_Articulos.findAll({
            limit: 10
        });
    },

    getTotalesPorQuincenas: async (opts?: { empresa?: number; articulo?: number; today?: string | Date; }) => {
        const sequelize = Movimientos_Articulos.sequelize!;
